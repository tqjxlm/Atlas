...
void CrowdSimulation::initStage(osg::ref_ptr<osg::Node> baseBound, osg::ref_ptr<osg::Vec3Array> edgePoints, bool addToScene)
{
	...
	for each (const auto& row in cells)
	{
		for each (const auto cell in row)
		{
...
void CrowdSimulation::onDoubleClick()
{
	if (_isDrawing)
	{
		...
		case PATH:
		{
			for each (const osg::Vec3& pickedPoints in *_pickedPoints)
			{
...
void CrowdSimulation::drawPath()
{
	...
	for each(Ped::Cell* node in lastAgentSource->group->currentPath)
	{
