...
{
public:
	RendererStepCallBack(
		CrowdSimRenderer* renderer, 
...
{
	_debugMode = enable;
	for each (AgentInstance agent in _agents)
	{
		if (enable)
...
void CrowdSimRenderer::generateAgents(AgentGenerator* source)
{
	...
	for each(const Ped::Agent* agent in newAgents)
	{
		osg::ref_ptr<osg::PositionAttitudeTransform> node = new osg::PositionAttitudeTransform;
...
void CrowdSimRenderer::startSimulation()
{
	...
	for each (AgentGenerator* source in _sources)
	{
