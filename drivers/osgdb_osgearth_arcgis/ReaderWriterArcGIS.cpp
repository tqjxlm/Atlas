...
public:
                                         ...
                                         ProgressCallback* progress)
    {
		...
		float high = _map_service.getHigh();
		float low = _map_service.getLow();
		float scaleFactor = 255 / (high - low);

		osg::HeightField *hf = new osg::HeightField();
		hf->allocate( image->s(), image->t() );

		for (int i = 0; i < hf->getNumColumns(); i++)
		{
			for (int j = 0; j < hf->getNumRows(); j++)
			{
				short r = *image->data(i, j);
				short g = *image->data(i+1, j);
				short b = *image->data(i+2, j);
				short gray = (r*19595 + g*38469 + b*7472) >> 16;

				hf->setHeight(i, j,  gray / scaleFactor + low);
			}
		}
