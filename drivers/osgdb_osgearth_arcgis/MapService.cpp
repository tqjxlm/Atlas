...
//    if ( !reader.parse( response.getPartStream(0), doc ) )
	...
	else
	{
		...
		ogrSRS.importFromEPSG(srs);
		char* proj4;
		ogrSRS.exportToProj4(&proj4);
		spatialReference = SpatialReference::get( proj4 );
		CPLFree(proj4);
	}
	...
	json_url = uri.full() + "/legend" + sep + std::string("f=pjson");  // request the data in JSON format
	r = URI(json_url).readString( options );
	...
	if ( !j_layers.empty() )
	{
		for( unsigned int i=0; i<j_layers.size(); i++ )
		{
			...
			Json::Value legend = layer["legend"];
			for ( unsigned int j=0; j<legend.size(); j++)
			{
				Json::Value entry = legend[j];
				Json::Value label = entry["label"];
				std::string name;
				std::string buf;
				float value;
				std::stringstream ss(label.asCString());
				ss >> name >> buf >> value;
				if (name == "High")
					high = value;
				else if (name == "Low")
