...
{
	DO_FOR_ALL_CELLS([](Cell* cell) { cell->updateAttribute(); });
}

void Ped::Grid::resetCellChangeList()
{
	DO_FOR_ALL_CELLS([](Cell* cell) { cell->resetChangeList(); });
}
...
void Grid::addAgent(const Agent * agent)
{
	...
	if (cell == NULL)
		throw exception("FATAL: Agent goes out of stage!");
...
const Cell::CellAttribute& Grid::getCellAttribute(double x, double y) 
{
	Cell* cell = getCellByPosition(x, y);
	if (cell == NULL)
		throw exception("Cell index OOR!");
