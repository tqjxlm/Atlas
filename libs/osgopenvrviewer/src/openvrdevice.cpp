...
void OpenVRDevice::ProcessVREvent(const vr::VREvent_t& event)
{
	...
	case vr::VREvent_ButtonPress:
	{
		...
		// 直接返回touchpad的坐标
		if (event.data.controller.button == vr::k_EButton_SteamVR_Touchpad && controllerRole == vr::TrackedControllerRole_RightHand)
		{
			m_vrSystem->GetControllerState(event.trackedDeviceIndex, &state, sizeof(state));
			...
			std::cout << "controllerEventResult:" << controllerEventResult << std::endl;
		}
	...
	case vr::VREvent_ButtonUnpress:
	{
			...
			controllerRole == vr::TrackedControllerRole_LeftHand)
		{
			...
			std::cout << "trigger unpress!!" << std::endl;
		}
	}
	break;
	}
